...
func read() (tiles Tiles, minY, maxY int) {
	lines := input.Lines()
...
func process(tiles Tiles, minY, maxY int) int {
	count := countWaterTiles(tiles, math.NewVector2(500, 1), minY, maxY)
	fmt.Println(tiles)

	return count
}

func countWaterTiles(tiles Tiles, pos math.Vector2[int], minY, maxY int) int {
	...
	below := pos.Add(Down)
	count := countWaterTiles(tiles, below, minY, maxY)
	if pos.Y >= minY {
		count++
	}
