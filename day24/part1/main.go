...
func process(immuneGroups, infectionGroups []Group) int {
	for len(immuneGroups) > 0 && len(infectionGroups) > 0 {
		...
		attack(immuneGroups, infectionGroups, immuneTargets, infectionTargets)

		immuneGroups = removeDefeatedGroups(immuneGroups)
		infectionGroups = removeDefeatedGroups(infectionGroups)
	}

	survived := immuneGroups
	if len(survived) == 0 {
		survived = infectionGroups
	}

	return slice.Sum(survived, func(g Group) int { return g.units })
}
...
func attack(immuneGroups, infectionGroups []Group, immuneTargets, infectionTargets []*int) {
...
func (g Group) Attack(defender *Group) {
	defender.units = max(0, defender.units-defender.TakenDamage(g)/defender.unitHP)
}
