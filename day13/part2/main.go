...
func process(paths [][]rune, carts []Cart) string {
	for len(carts) > 1 {
		sortCarts(carts)
		positions := make(map[math.Vector2[int]]int, len(carts))
		for i, cart := range carts {
			positions[cart.position] = i
		}
		crashedCarts := map[int]bool{}
		for i := range carts {
			if crashedCarts[i] {
				continue
			}
			...
			moveCart(paths, cart)
			pos := cart.position
			if cartIdx, ok := positions[pos]; ok {
				delete(positions, pos)
				crashedCarts[i] = true
				crashedCarts[cartIdx] = true
				continue
			}
			positions[cart.position] = i
		}
		...
		for i, cart := range carts {
			if !crashedCarts[i] {
				nextCarts = append(nextCarts, cart)
			}
		}
		carts = nextCarts
	}

	pos := carts[0].position
